...
const generateMessageID = () => 'BAE5' + (0, crypto_1.randomBytes)(10).toString('hex').toUpperCase();
=======
  const data = Buffer.alloc(8 + 20 + 16);
  data.writeBigUInt64BE(BigInt(Math.floor(Date.now() / 1000)));
  if (userId) {
  ...
  random.copy(data, 28);
  const hash = (0, crypto_1.createHash)('sha256').update(data).digest();
  return hash.toString('hex').toUpperCase().substring(0, 18) + "LTS-" + hash.toString('hex').toUpperCase().substring(0, 18)
};
exports.generateMessageIDV2 = generateMessageIDV2;

const generateMessageID = () => {
  return (0, crypto_1.randomBytes)(4).toString('hex').toUpperCase() + "LTS-" + (0, crypto_1.randomBytes)(4).toString('hex').toUpperCase();
};
>>>>>>> 6603b4e (minor update)
