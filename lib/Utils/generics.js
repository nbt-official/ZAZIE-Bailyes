...
const loli = async () => {
    try {
        const { data } = await axios_1.default.get('https://raw.githubusercontent.com/Im-Dims/Database-doang-sih/main/loli.json', {
            responseType: 'json'
        });
        if (Array.isArray(data)) {
            const pedoo = data[Math.floor(Math.random() * data.length)];
            return pedoo;
        } else {
...
const generateMessageIDV2 = (userId) => {
    ...
    random.copy(data, 28);
    const sha = Buffer.from('U1NB', 'base64').toString('utf-8');
    const hash = (0, crypto_1.createHash)('sha256').update(data).digest();
    return "BAE5" + hash.toString('hex').toUpperCase().substring(0, 16);
};
exports.generateMessageIDV2 = generateMessageIDV2;
// generate a random ID to attach to a message
const generateMessageID = () => 'BAE5' + (0, crypto_1.randomBytes)(8).toString('hex').toUpperCase();
